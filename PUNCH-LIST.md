...
| 🔵 [CER-1149](https://linear.app/cerebral-work/issue/CER-1149) | P4 | `feat(operator)`: `linearctl operator` long-running daemon — CF Queue `linear-agent-events` consumer + Unix-socket control (`POST /delegate`, `GET /healthz`) for `linearctl watch` delegation; token cache; graceful SIGINT/SIGTERM shutdown; on-branch |
